...
//! Does nothing more than sticking incoming data into an orcaiceutil::Buffer.
//!
...
public:
    //! Constructor.
    BufferedConsumerImpl( int depth, orcaiceutil::BufferType type, const orcaice::Context &context )
        : ConsumerImpl<ProviderPrxType,ConsumerType,ConsumerPrxType,ObjectType>(context),
          buffer_(depth,type) {}

    //! Returns reference to local proxy.
    orcaiceutil::Buffer<ObjectType> &buffer() { return buffer_; }
...
private:
    orcaiceutil::Buffer<ObjectType> buffer_;
};
