...
//
// this file is a modified copy of ZeroC's Application.cpp v.3.1.0
// all Ice::Application replaced with orcaice::IceApplication
// for other mod's search for 'orca'
//

//orca #include <Ice/Application.h>
#include <orcaice/iceapplication.h>
static bool _useCtrlCHandler = false;
...
// orca start
// orca Ice::Application::Application()
// orca {
// orca }
orcaice::IceApplication::IceApplication( bool installCtrlCHandler )
{
    _useCtrlCHandler = installCtrlCHandler;
}
// orca end

orcaice::IceApplication::~IceApplication()
{
}

int
orcaice::IceApplication::main(int argc, char* argv[])
{
    return main(argc, argv, InitializationData());
}

int
orcaice::IceApplication::main(int argc, char* argv[], const char* configFile)
{
    //
...
int
orcaice::IceApplication::main(int argc, char* argv[], const char* configFile, const Ice::LoggerPtr& logger)
{
    InitializationData initData;
    if(configFile)
    {
...
int
orcaice::IceApplication::main(int argc, char* argv[], const InitializationData& initData)
{
    if(_communicator != 0)
    {
...
// orca start
//
// orca: this big ugly if statement is needed because the try/catch extends so far
// orca: tried to move catch right below the first try but then Ctrl-C has no effect.
// orca: don't understand why.
//
if ( _useCtrlCHandler )
{
// orca end
...
// orca start
else // if ( !_useCtrlCHandler )
{
// orca: this is a copy of what's above with stuff related to ctrlCHandler commented out
/* orca
    try
    {
	//
...
orca */
} // orca: the end end of our ugly IF statement
// orca end
    return status;
}

const char*
orcaice::IceApplication::appName()
{
    return _appName;
}

CommunicatorPtr
orcaice::IceApplication::communicator()
{
    return _communicator;
}

void
orcaice::IceApplication::destroyOnInterrupt()
{   
    //
...
void
orcaice::IceApplication::shutdownOnInterrupt()
{
    if(_ctrlCHandler != 0)
    {
	StaticMutex::Lock lock(_mutex); // we serialize all the interrupt-setting
...
void
orcaice::IceApplication::ignoreInterrupt()
{
    if(_ctrlCHandler != 0)
    {
	StaticMutex::Lock lock(_mutex); // we serialize all the interrupt-setting
...
void
orcaice::IceApplication::holdInterrupt()
{
    if(_ctrlCHandler != 0)
    {
	StaticMutex::Lock lock(_mutex); // we serialize all the interrupt-setting
...
void
orcaice::IceApplication::releaseInterrupt()
{
    if(_ctrlCHandler != 0)
    {
	StaticMutex::Lock lock(_mutex); // we serialize all the interrupt-setting
...
bool
orcaice::IceApplication::interrupted()
{
    StaticMutex::Lock lock(_mutex);
    return _interrupted;
}
