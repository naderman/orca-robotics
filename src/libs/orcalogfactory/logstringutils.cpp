...
Sept/2008 Michael: changed to one line format, to make replaying possible again

timestamp in seconds and microseconds of Unix time {3D pose} {3D velocities}
...
void
fromLogString( stringstream &s, orca::Odometry3dData &obj )
{
    fromLogString(s,obj.timeStamp);
    double tmp;
    s >> obj.pose.p.x 
      >> obj.pose.p.y 
      >> obj.pose.p.z ;
    s >> tmp; obj.pose.o.r = DEG2RAD(tmp);
    s >> tmp; obj.pose.o.p = DEG2RAD(tmp);
    s >> tmp; obj.pose.o.y = DEG2RAD(tmp);
    s >> obj.motion.v.x 
      >> obj.motion.v.y 
      >> obj.motion.v.z ;
    s >> tmp; obj.motion.w.x = DEG2RAD(tmp);
    s >> tmp; obj.motion.w.y = DEG2RAD(tmp);
    s >> tmp; obj.motion.w.z = DEG2RAD(tmp);
}
