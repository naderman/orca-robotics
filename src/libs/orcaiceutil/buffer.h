...
#ifndef ORCAICEUTIL_BUFFER_H
#define ORCAICEUTIL_BUFFER_H

#include <queue>
#include <orcaiceutil/common.h>
...
@verbatim
orcaiceutil::Buffer<double> buffer( 10, orcaiceutil::BufferTypeCircular );
double data;
while (1)
{
    try {
...
public:
     ...
     *
     *  Calls to @ref get() on an empty buffer raises an orcaiceutil::Exception exception.
...
void Buffer<Type>::get( Type &obj ) const
{
    ...
    {
        throw orcaiceutil::Exception( ERROR_INFO, "trying to read from an empty buffer." );
    }
}
...
void Buffer<Type>::get( Type &obj, unsigned int n ) const
{
    ...
    if ( queue_.empty() ){
        ...
        throw orcaiceutil::Exception( ERROR_INFO, "index out of bounds while trying to read buffer." );
    }
    else{
        internalGet( obj ,n );
    }
