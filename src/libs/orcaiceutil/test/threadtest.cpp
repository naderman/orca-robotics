...
class TestThread : public orcaiceutil::Thread
{
public:    
    virtual void run()
    {
...
class TestThreadWithThrow : public orcaiceutil::Thread
{
public:
...
class TestThreadWithExit : public orcaiceutil::Thread
{
public:  
    TestThreadWithExit() : 
        isExiting_(false) {};
...
class TestThreadWithWait : public orcaiceutil::Thread
{
public:
    virtual void run()
    {
...
int main(int argc, char * argv[])
{
    ...
    cout<<"testing state machine ... ";
    {
        ...
        orcaiceutil::ThreadPtr t = new TestThreadWithExit;
    ...
    {
        orcaiceutil::ThreadPtr t = new TestThreadWithWait;
        t->start();
        if ( t->isAlive()!=true ) {
            cout<<"failed"<<endl
                <<"should be in Started state but internal states do not match:"<<endl
...
//     cout<<"ok"<<endl;
    ...
    {
        orcaiceutil::ThreadPtr t = new TestThread;
        t->start();
        if ( t->isAlive()!=true ) {
            cout<<"failed"<<endl
                <<"should be in Started state but internal states do not match:"<<endl
