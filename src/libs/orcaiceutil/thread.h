...
#ifndef ORCAICEUTIL_THREAD_H
#define ORCAICEUTIL_THREAD_H
...
The implicit state machine of this class has 4 states {Starting, Running, Stopping, Stopped}. Events and the corresponding state transitions are listed below. Note that the final state may only be accessed when using smart pointer orcaiceutil::ThreadPtr (otherwise the thread self-destructs).
...
- orcaiceutil::Threads self-destruct (ie call their own destructor) when orcaiceutil::Thread::run returns, unless you hold onto an orcaiceutil::ThreadPtr which points to it. So never call @c delete on the pointer to a thread, doing so will result in segmentation fault.
...
void stopAndJoin( orcaiceutil::Thread* thread );

//! A convenience function which first stops the @p thread and then waits for it to
//! terminate. If the smart pointer is 0, this function quietly returns.
void stopAndJoin( orcaiceutil::ThreadPtr thread );
