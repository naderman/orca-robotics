...
#ifndef ORCAICEUTIL_NOTIFY_H
#define ORCAICEUTIL_NOTIFY_H
...
public:
    ...
    //! Raises orcaiceutil::Exception if the function is called when a notify handler has
    //! not been set.
...
void Notify<Type>::set( const Type & obj )
{
    if ( !hasNotifyHandler_ ) {
        throw orcaiceutil::Exception( ERROR_INFO, "setting data when data handler has not been set" );
    }
