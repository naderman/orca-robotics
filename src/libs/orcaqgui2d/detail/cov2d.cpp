...
void Cov2d::eigvect( const double & d1,
                     const double & d2,
                     double & v1x,
                     double & v1y,
                     double & v2x,
                     double & v2y ) const
{
#define USE_TIMBAILEY_METHOD false
#if USE_TIMBAILEY_METHOD
    //
    // AlexB: This code is suspect...
    //        It breaks for ellipses at 45deg from axis-aligned.
    //
    ...
    // Special case of axis-aligned
    if ( fabs(xy_) < (fabs(xx_) + fabs(yy_) + 1e-4) * 1e-4 )
    {
        if ( fabs(d1-xx_) < 1e-5 )
        {
            v1x = 1;
