...
PathFollower2dElement::isFollowerEnabled( bool &isEnabled )
{
    try
    {
        ...
        if (isRemoteInterfaceSick_) {
            _stuff.humanManager->showStatusInformation( "PathFollower2dElement::remote interface is sane again" );
            isRemoteInterfaceSick_ = false;
        }
        return 0;
    }
    ...
    {
        if (!isRemoteInterfaceSick_) {
            _stuff.humanManager->showStatusError(  "PathFollower2dElement::remote interface seems sick" );
...
PathFollower2dElement::connectToInterface()
{
    ...
    _stuff.humanManager->showStatusInformation( "PathFollowerElement is trying to connect");
    ...
    try 
    {
        ...
        _stuff.humanManager->showStatusInformation( "Connected to pathfollower interface successfully.");
        isConnected_ = true;
    }
    catch ( const std::exception &e )
    {
        ...
        _stuff.humanManager->showStatusWarning( ss.str().c_str() );
    }
    catch ( ... )
    {
        _stuff.humanManager->showStatusWarning( "PathFollower2dElement: Unknown problem connecting to pathfollower interface.");
    }
}
...
PathFollower2dElement::toggleEnabled()
{
    if ( !pathFollower2dPrx_ ) {
        _stuff.humanManager->showStatusInformation("Proxy to PathFollower2d interface is not initialized");
        return;
    }
    ...
    _stuff.humanManager->showStatusInformation(str);
}
...
{
    if (!pathFollower2dPrx_) {
        _stuff.humanManager->showStatusError("Proxy to pathfollower2d interface is not intitialized");
        return;
    }
    
//     cout<<"TRACE(PathFollower2dElement): go()" << endl;
    _stuff.humanManager->showStatusInformation("Received GO signal");
    try
    {
    ...
    catch ( const std::exception &e )
    {
        ...
        _stuff.humanManager->showStatusError( QString(ss.str().c_str()) );
    }
}
...
//     cout<<"TRACE(PathFollower2dElement): stop()" << endl;
    _stuff.humanManager->showStatusInformation("Received STOP signal");
    orca::PathFollower2dData dummyPath;
    dummyPath.timeStamp = orcaice::getNow();
    const bool activateNow = true;
    try
    {
        cout<<"TRACE(pathfollower2delement.cpp): setData() with data: " << orcaobj::toVerboseString(dummyPath) << endl;
    ...
    try
    {
        ...
        if ( guiPath.size()>0 )
        {
            ...
            if (!activateImmediately) 
                _stuff.humanManager->showStatusInformation( "Path needs to be activated by pressing the Go button." );
        } 
        else 
        {
            _stuff.humanManager->showStatusWarning( "No path to send!" );
        }        
    }
    catch ( const std::exception &e )
    {
