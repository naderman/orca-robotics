...
using namespace std;
using namespace cameracollectionviewocv;
...
MainThread::work() 
{
    ...
    Viewer viewer = Viewer( cameraCollectionData_,
                            context_ );
    ...
    while ( !isStopping() )
    {
        try 
        {
            ...
            viewer.display( cameraCollectionData_ );
...
MainThread::initNetworkInterface()
{
    ...
    //
    cameraCollectionInterface_ = new orcaifaceimpl::BufferedCameraCollectionConsumerImpl( 1, gbxiceutilacfr::BufferTypeCircular, context_ );

    //
    // SUBSCRIBE
    //
    // try multiple times
    cameraCollectionInterface_->subscribeWithTag( "CameraCollection", this, subsysName() );
}
...
void MainThread::getImages()
{
    ...
    int ret = cameraCollectionInterface_->buffer().getAndPopNext( cameraCollectionData_, timeoutMs );
