...
{	
    ...
    int n,i,m=0,n0 ;
	n = SerialReadJEG( FComLsr,  s, 200, 200, T5segs/10 );
	n0=n ;
	if (n>6){	n=6 ; }
...
{	
    ...
    static char str[1000] ;
	int n,i,m=0,n0 ;
	n = SerialReadJEG( FComLsr,  s, 1000, 1000, T5segs/5 );
	n0=n ;
	if (n>10){	n=10 ; }
	for (i=0;i<n;i++){	m=m+sprintf(str+m,"%x|",0x00FF&(unsigned)s[i]) ; }
	str[m]='@' ; str[m+1]=0 ; 
...
void SetLaser(int FComLsr,struct Laser *pg)
{
	...
	// set to 'install' mode
	SerialWriteJEG( FComLsr, string1, STR1, &hkswrite );
	...
	SerialWriteJEG( FComLsr, string2b, STR2, &hkswrite);
	...
	// more initialisation
	// laser configuration string
	SerialWriteJEG(FComLsr, string4b, STR4, &hkswrite);
	...
	SerialWriteJEG(FComLsr, stringAskInfo1, sizeof(stringAskInfo1), &hkswrite);
	...
	SerialWriteJEG(FComLsr, string5, STR5, &hkswrite);
...
static void readSickLaser(struct Laser *pxl,int *pFlag)
{
	...
	while(*pFlag)
	{
		...
		if (xx >=0)
		{
			...
			nnn = SerialReadJEG(FComLsr, string+1, 3, 3, T5segs);
			...
			if ( (nnn==3) && (string[1] == '\x80'))
			{
                ...
                nnn = SerialReadJEG(FComLsr, string+4, 728, 728, T5segs);
