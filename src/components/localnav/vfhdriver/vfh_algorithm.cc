...
VFH_Algorithm::VFH_Algorithm( const VfhAlgorithmConfig &config )
    : CELL_WIDTH((int)(config.cellSize*1000.0)),
      ...
      SECTOR_ANGLE((int)(config.sectorAngle*180.0/M_PI)),
      SAFETY_DIST_0MS(config.safetyDist0ms*1000.0),
      SAFETY_DIST_1MS(config.safetyDist1ms*1000.0),
      Current_Max_Speed((int)(config.maxSpeed*1000.0)),
      MAX_SPEED((int)(config.maxSpeed*1000.0)),
      MAX_SPEED_NARROW_OPENING((int)(config.maxSpeedNarrowOpening*1000.0)),
      MAX_SPEED_WIDE_OPENING((int)(config.maxSpeedWideOpening*1000.0)),
      MAX_ACCELERATION((int)(config.maxAcceleration*1000.0)),
      MAX_TURNRATE_0MS((int)(config.maxTurnrate0ms*180.0/M_PI)),
      MAX_TURNRATE_1MS((int)(config.maxTurnrate1ms*180.0/M_PI)),
      TURNRATE_THRESHOLD(99999),
      MIN_TURN_RADIUS_SAFETY_FACTOR(config.minTurnRadiusSafetyFactor),
      Binary_Hist_Low_0ms(config.freeSpaceCutoff0ms),
      Binary_Hist_High_0ms(config.obsCutoff0ms),
      Binary_Hist_Low_1ms(config.freeSpaceCutoff1ms),
      Binary_Hist_High_1ms(config.obsCutoff1ms),
      U1(config.weightDesiredDir),
      U2(config.weightCurrentDir),
      ROBOT_RADIUS((int)(config.robotRadius*1000.0)),
      Desired_Angle(90),
      Picked_Angle(90),
...
{ 
    ...
    if ( val > TURNRATE_THRESHOLD ) val = TURNRATE_THRESHOLD;
...
VFH_Algorithm::SetCurrentMaxSpeed( int max_speed )
{
    ...
    if ( max_speed > this->MAX_SPEED ) this->Current_Max_Speed = this->MAX_SPEED;
}
...
int
VFH_Algorithm::MinTurningRadius( int speed )
{
    double dx = (double) speed / 1e6; // dx in m/millisec
    double dtheta = ((M_PI/180)*(double)(GetMaxTurnrate(speed))) / 1000.0; // dTheta in radians/millisec
    return (int) ( ((dx / tan( dtheta ))*1000.0) * MIN_TURN_RADIUS_SAFETY_FACTOR ); // in mm
}
...
int VFH_Algorithm::Update_VFH( double laser_ranges[REQUIRED_NUM_RANGES][2], 
                               int current_speed, 
...
bool VFH_Algorithm::Cant_Turn_To_Goal()
{
    ...
    float goal_x = this->Dist_To_Goal * cos( DEG2RAD(this->Desired_Angle) );
    float goal_y = this->Dist_To_Goal * sin( DEG2RAD(this->Desired_Angle) );
...
int VFH_Algorithm::Calculate_Cells_Mag( double laser_ranges[REQUIRED_NUM_RANGES][2], int speed ) 
{
  int x, y;
...
int VFH_Algorithm::Build_Masked_Polar_Histogram(int speed) 
{
  ...
  int min_turning_radius = MinTurningRadius( speed );
  center_x_right = CENTER_X + (min_turning_radius / (float)CELL_WIDTH);
  center_x_left = CENTER_X - (min_turning_radius / (float)CELL_WIDTH);
  center_y = CENTER_Y;
  ...
  Blocked_Circle_Radius = min_turning_radius + ROBOT_RADIUS + Get_Safety_Dist(speed);
