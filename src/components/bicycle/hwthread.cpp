...
HwThread::work()
{
    ...
    while( !isStopping() )
    {
        ...
        {
            hydrointerfaces::Bicycle::Command command;
...
void
HwThread::setCommand( const hydrointerfaces::Bicycle::Command &command )
{
...
bool
HwThread::getLastCommand( hydrointerfaces::Bicycle::Command &command ) const
{
    // If store is empty, just return false
    if( commandStore_.isEmpty() )
    {
	return false;
    }

    // Return the last received command
    commandStore_.peek( command );
    return true;
}
