...
*/    
    ...
    {
        stringstream ss;
        ss << "RawRxData::"<<__func__<<"(): Received CAN packet with id 0x"<<std::hex<<pkt.id()<<".  Don't know what to handle this!";
        throw hydrointerfaces::SegwayRmp::Exception( ss.str() );
        break;
    }
    }
...
RawRxData::toString() const
{
    ...
    haveMonitorMsg_[4] ? os<<setw(17)<<"yaw"<<dec<<setw(10)<<(int)yaw<<hex<<setw(10)<<yaw<<"\t\t"<<dec<<(int)yaw/RMP_COUNT_PER_DEG<<" deg"<<endl : os<<"yaw        \tDROPPED"<<endl;
