...
addGQG( double &Pxx,
        ...
        double  cTheta )
{
    // E = velScaleCov*abs(actX)
    // F = velScaleCov*abs(actY)
    //
    // GQG =
    // [ cPsi*E*cPsi+sPsi*F*sPsi, cPsi*E*sPsi-sPsi*F*cPsi,           0           ]
    // [ sPsi*E*cPsi-cPsi*F*sPsi, sPsi*E*sPsi+cPsi*F*cPsi,           0           ]
    // [           0,                       0,                yawScale*abs(actT) ]

    double E = velScaleCov*fabs(actX);
    double F = velScaleCov*fabs(actY);
    ...
    double GQGyt = 0;
    double GQGtt = yawScaleCov*fabs(actT);
...
//     printP( Pxx, Pxy, Pyy, Pxt, Pyt, Ptt );
    ...
    addGQG(   Pxx, Pxy, Pyy, Pxt, Pyt, Ptt, actX, actY, actT, velScaleCov, yawScaleCov, sTheta, cTheta );
