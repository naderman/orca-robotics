...
MainThread::walk()
{
    ...
    {
        context_.shutdown();
        throw hydroutil::Exception( ERROR_INFO, "Unknown ReplayController: "+replayController );
    }
...
MainThread::loadPluginLibraries( const std::string & factoryLibNames )
{
    ...
    if ( replayerFactories_.empty() ) {
        context_.shutdown();
        throw hydroutil::Exception( ERROR_INFO, "No replayer factories were loaded." );
    }
}
    ...
    for ( unsigned int i=0; i < replayerFactories_.size(); ++i )
    {
        ...
        try
        {
            ...
            if ( s ) { 
                ...
                context_.shutdown();
                throw hydroutil::Exception( ERROR_INFO, "Error when creating replayer for supported interface type " + interfaceType );
            }
        }
        catch ( const orcalog::FormatNotSupportedException& e )
        {
            ...
            {
                context_.shutdown();
                throw hydroutil::Exception( ERROR_INFO, "Format "+format+" is not supported for interface type"+interfaceType );
            }
            else {
                replayers_.push_back( new orcalog::DummyReplayer() );
