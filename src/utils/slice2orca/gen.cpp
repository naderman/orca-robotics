...
//     string lib_namespace = _module + base_namespace;
    ...
    // include the util files generated by slice2orca from the Slice files included from this Slice file
...
//     cout<<"DEBUG: Gen::visitClassDefStart()"<<endl;
    ...
    C << nl << "if ( objPtr == 0 )";
    C << sb;
    C << nl << "throw gbxutilacfr::Exception( ERROR_INFO, \"(while logging class " << scoped << ") cannot process a null pointer\" );";
    C << eb;
...
{
    bool isTopModule = topModule_.empty();
...
{
    assert( !topModule_.empty() );

    bool isTopModule = ( topModule_ == p->name() );

    // only write something when closing the top-level module
    if ( !isTopModule )
        return;

    // alexm: I think this comment is out-of-date.
